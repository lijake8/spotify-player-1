...
def index():
    ...
    if request.args.get("code"): #access parameter '?code=' in the URL
        ...
        print('entered into first if statement!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        auth_manager.get_access_token(request.args.get("code"))
        # return redirect('/')

    if not auth_manager.validate_token(cache_handler.get_cached_token()):
        # Step 1. Display spotify sign in link when no token
        print('entered into second if statement!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        auth_url = auth_manager.get_authorize_url()
        # https://accounts.spotify.com/authorize?client_id=${CLIENT_ID}&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A5000&scope=user-read-currently-playing+playlist-modify-private&show_dialog=True
        # auth_url = f'https://accounts.spotify.com/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri=${REDIRECT_URI}&scope={}'
...
def playlists():
    cache_handler = spotipy.cache_handler.FlaskSessionCacheHandler(session)
    auth_manager = spotipy.oauth2.SpotifyOAuth(client_id=os.getenv('CLIENT_ID'),
                                               client_secret=os.getenv('CLIENT_SECRET'),
                                               redirect_uri=os.getenv('REDIRECT_URI'), 
                                               cache_handler=cache_handler)
    if not auth_manager.validate_token(cache_handler.get_cached_token()):
